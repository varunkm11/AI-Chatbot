...
try:
    ...
    st.warning("⚠️ Real-time data features not available. Install dependencies for full functionality.")
...
def init_session_state():
    ...
    if REALTIME_AVAILABLE and 'realtime_provider' not in st.session_state:
...
def main():
    ...
    <div class="main-header">
        ...
        <p><small>Deployed on Heroku</small></p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        # Get API Key from environment variables (Heroku config vars)
        ...
        if api_key:
            ...
            st.info("Set OPENROUTER_API_KEY in Heroku config vars")
            api_key = st.text_input("Enter API Key temporarily:", type="password")
    ...
    with col2:
        ...
        if REALTIME_AVAILABLE:
            ...
            realtime_enabled = st.checkbox("Enable real-time information", value=True)
        else:
            st.warning("⚠️ Real-time features limited")
            st.info("💡 Try asking about:\n- General questions\n- Programming help\n- Creative writing\n- Problem solving")
    ...
    if send_button and user_input.strip():
        if not api_key:
            st.error("⚠️ API Key required. Set OPENROUTER_API_KEY in Heroku config vars.")
            return
        ...
        with st.spinner("🤔 AI is thinking..."):
            ...
            if REALTIME_AVAILABLE and realtime_enabled:
    ...
    <div style="text-align: center; color: #666; padding: 1rem;">
        <p>🤖 Advanced AI Chatbot | Deployed on Heroku</p>
